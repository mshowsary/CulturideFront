<template>
  <section class="">
  <div class="container max-w-full">
    <div class="row">
      <div class="min-h-[980px] bg-white py-8 lg:col-6 lg:py-[114px]">
       
        <div class="mx-auto w-full max-w-[480px]">
          <h3 class="mb-4 pb-8">SE CONNECTER</h3>          
          <div class="p-8">
            <span v-if="error.show" class="text-red-600">{{ error.msg }}</span>            
          </div> 
          <form action="#" @submit.prevent="login">           
            <div class="form-group mt-4">              
              <input
                type="email"
                id="email"
                class="form-control"
                placeholder="Adresse e-mail"
                v-model="form.email"
              />
            </div>
            <div class="form-group mt-4">
              
              <input
                type="password"
                id="password"
                class="form-control"
                placeholder="Mot de passe"
                v-model="form.password"
              />
            </div>
            <div class="form-group mt-4 flex justify-between mt-4">
              <NuxtLink href="/signup" class="text-blue-400">S'INSCRIRE</NuxtLink>
              <NuxtLink href="/reset-password" class="text-blue-400">MOT DE PASSE OUBLIÉ</NuxtLink>
            </div> 
            <input
              class="btn btn-primary mt-10 block w-full"
              type="submit"
              value="Se Connecter"
            />
          </form>
        </div>
      </div>

      <div
        class="auth-banner flex flex-col items-center justify-center py-16 lg:col-6"
      >
        <img
          class="absolute top-0 left-0"
          src="/signup2.jpg"
          alt=""
        />
      </div>
    </div>
  </div>
</section>
  </template>

  <script setup>
  import {reactive} from 'vue'
  import {useStore, useGoTo, useAccount} from '~/stores/token'
  useHead({
    title: 'CULTURIDE - SIGN IN'
    }) 
  const store = useStore();
  const storeG = useGoTo();
  const storeA = useAccount()

  const error = reactive({show:false, msg:''});
  const form = reactive({email: '', password:''})

  definePageMeta({
    middleware: [
      function (to, from) {},
      "auth"
      ],
  });

    async function login(){
      await $fetch('http://127.0.0.1:8000/api/v1/login', {
        method: 'POST',
        body: form
        }).then(res => {
          if(res.success)
          { 
            form.password = null
            form.email = null

            localStorage.setItem('token', res.token)
            store.token = res.token
            storeA.name = res.name

            navigateTo(storeG.url ?? "/dashboard/tickets")
            storeG.url = "/dashboard/tickets"
            
            error.show = false
            error.msg = ''
          }else{
            
              error.show = true
              error.msg = res.msg  
          }
        }).catch(er => {
          console.log(er.data)
          error.show = true
          error.msg = er.data.msg
        })
    }

  </script>